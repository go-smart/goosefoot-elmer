    ...
    INTERFACE
      ...
      SUBROUTINE SolveHYPRE1( n, Rows, Cols, Vals, Precond, PrecVals, GDOFs, &
           Owner, ILUn, BILU, hypremethod, hypre_intpara, hypre_dppara, rounds, &
               TOL, verbosity, hypreContainer, fcomm )
        USE, INTRINSIC :: iso_c_binding
...
#ifdef HAVE_TRILINOS
    INTERFACE
      !! create TrilinosEpetra matrix and setup solver/preconditioner
      SUBROUTINE SolveTrilinosEpetra1( n, nnz, Rows, Cols, Vals, &
           GDOFs, Owner, &
           xmlfilename, verbosity, triliContainer, n_nodes, &
           xcoords,ycoords,zcoords,ierr) BIND(C,name='SolveTrilinosEpetra1')
      ...
      END SUBROUTINE SolveTrilinosEpetra1

      !! solve linear system with same matrix as in SolveHYPRE1
      SUBROUTINE SolveTrilinosEpetra2( n, Xvec, RHSVec, Rounds, TOL, &
      verbosity, triliContainer, ierr) &
                BIND(C,name='SolveTrilinosEpetra2')
        USE, INTRINSIC :: iso_c_binding
        INTEGER(KIND=c_int) :: n, Rounds, verbosity
        REAL(KIND=c_double) :: Xvec(n),RHSvec(n),TOL
        INTEGER(KIND=C_INTPTR_T) :: triliContainer
        INTEGER(KIND=C_INT) :: ierr
      END SUBROUTINE SolveTrilinosEpetra2

      !! note: SolveTrilinosEpetra3 does not yet exist, it would update the matrix
      !!       but not the preconditioner.

      !! destroy the data structures (should be called when the matrix has
      !! to be updated and SolveTrilinosEpetra1 has to be called again).
      SUBROUTINE SolveTrilinosEpetra4(triliContainer) BIND(C,name='SolveTrilinosEpetra4')
        USE, INTRINSIC :: iso_c_binding
        INTEGER(KIND=C_INTPTR_T) :: triliContainer
      END SUBROUTINE SolveTrilinosEpetra4

    END INTERFACE

    INTERFACE
      !! create TrilinosKokkos matrix and setup solver/preconditioner
      SUBROUTINE SolveTrilinosKokkos1( n, nnz, Rows, Cols, Vals, &
           GDOFs, Owner, &
           xmlfilename, verbosity, triliContainer, n_nodes, &
           xcoords,ycoords,zcoords,ierr) BIND(C,name='SolveTrilinosKokkos1')
        ...
        REAL(KIND=c_double) :: xcoords(n_nodes), ycoords(n_nodes), zcoords(n_nodes)
        INTEGER(KIND=C_INT) :: ierr
      END SUBROUTINE SolveTrilinosKokkos1

      !! solve linear system with same matrix as in SolveHYPRE1
      SUBROUTINE SolveTrilinosKokkos2( n, Xvec, RHSVec, Rounds, TOL, &
      verbosity, triliContainer, ierr) &
                BIND(C,name='SolveTrilinosKokkos2')
        USE, INTRINSIC :: iso_c_binding
        INTEGER(KIND=c_int) :: n, Rounds, verbosity
        REAL(KIND=c_double) :: Xvec(n),RHSvec(n),TOL
        INTEGER(KIND=C_INTPTR_T) :: triliContainer
        INTEGER(KIND=C_INT) :: ierr
      END SUBROUTINE SolveTrilinosKokkos2

      !! note: SolveTrilinosKokkos3 does not yet exist, it would update the matrix
      !!       but not the preconditioner.

      !! destroy the data structures (should be called when the matrix has
      !! to be updated and SolveTrilinosKokkos1 has to be called again).
      SUBROUTINE SolveTrilinosKokkos4(triliContainer) BIND(C,name='SolveTrilinosKokkos4')
        USE, INTRINSIC :: iso_c_binding
        INTEGER(KIND=C_INTPTR_T) :: triliContainer
      END SUBROUTINE SolveTrilinosKokkos4
...
!     CALL ExchangeSourceVec( SourceMatrix, SplittedMatrix, ParallelInfo, RHSVec )
   ...
   END IF
    ...
    IF (ListGetLogical( Params,  &
           'Linear System Use TrilinosEpetra', Found )) THEN

      ! we attempt to read TrilinosEpetra settings from an XML file,
      ...
      xmlfile = ListGetString( Params, & 
          'TrilinosEpetra Parameter File', Found )
      IF (.NOT. Found) THEN
        xmlfile = 'none'
      END IF
      ...
      nnz = SourceMatrix%Rows(n+1) ! number of local nonzeros
        ...
        NewSetup=ListGetLogical( Params, 'Linear System Refactorize',Found ) 
        IF (NewSetup) THEN
          IF (SourceMatrix % TrilinosEpetra/=0) THEN
            CALL SolveTrilinosEpetra4(SourceMatrix % TrilinosEpetra)
          END IF
        END IF
        ! setup solver/preconditioner
        IF (SourceMatrix % TrilinosEpetra==0) THEN
          ...
          CALL SolveTrilinosEpetra1(n, nnz, & 
                SourceMatrix % Rows, &
                SourceMatrix % Cols, &
                SourceMatrix % Values, &
                ParallelInfo%GlobalDofs, Owner, &
                xmlfile, verbosity, SourceMatrix % TrilinosEpetra, &
                CurrentModel%Nodes%NumberOfNodes, &
                CurrentModel%Nodes%x, CurrentModel%Nodes%y, CurrentModel%Nodes%z, ierr)     

          DEALLOCATE( Owner )
          IF (ierr<0) THEN
            CALL Fatal('SParIterSolver','Failed to construct TrilinosEpetra solver')
          ELSE IF (ierr>0) THEN
            CALL Warn('SParIterSolver','Warning issued when trying to construct TrilinosEpetra solver')          
          END IF
        END IF
        ! solve using previously computed TrilinosEpetra data structures.
        ! NOTE: this is only correct if the matrix has not changed,
        ! otherwise we should use the SolveTrilinosEpetra3 function, which is
        ! not implemented, yet. This function will not update the matrix
        ! in the solver and thus solve an old system if A has changed. 
        CALL SolveTrilinosEpetra2( n, Xvec, RHSvec, &
                Rounds, TOL, verbosity, SourceMatrix % TrilinosEpetra, ierr)
      
      IF (ierr<0) THEN
        CALL Fatal('SParIterSolver',&
               'Linear system solve using TrilinosEpetra caused an error');
      ELSE IF (ierr>0) THEN
        IF (ListGetLogical(Solver%Values,'Linear System Abort Not Converged',Found)) THEN
          CALL Fatal('SParIterSolver', &
                'Linear system solve using TrilinosEpetra issued a warning');
        ELSE
...
#ifdef HAVE_TRILINOS
    IF (ListGetLogical( Params,  &
           'Linear System Use TrilinosKokkos', Found )) THEN

      ! we attempt to read TrilinosKokkos settings from an XML file,
      ! which is their usual way of getting parameters. If no 
      ! file is given, we use default settings and issue a    
      ! warning.
      xmlfile = ListGetString( Params, & 
          'TrilinosKokkos Parameter File', Found )
      IF (.NOT. Found) THEN
        xmlfile = 'none'
      END IF
      xmlfile = TRIM(xmlfile)//C_NULL_CHAR
      ...
      nnz = SourceMatrix%Rows(n+1) ! number of local nonzeros
        ...
        NewSetup=ListGetLogical( Params, 'Linear System Refactorize',Found ) 
        IF (NewSetup) THEN
          IF (SourceMatrix % TrilinosKokkos/=0) THEN
            CALL SolveTrilinosKokkos4(SourceMatrix % TrilinosKokkos)
          END IF
        END IF
        ! setup solver/preconditioner
        IF (SourceMatrix % TrilinosKokkos==0) THEN
          ...
          CALL SolveTrilinosKokkos1(n, nnz, & 
                SourceMatrix % Rows, &
                SourceMatrix % Cols, &
                SourceMatrix % Values, &
                ParallelInfo%GlobalDofs, Owner, &
                xmlfile, verbosity, SourceMatrix % TrilinosKokkos, &
                CurrentModel%Nodes%NumberOfNodes, &
                CurrentModel%Nodes%x, CurrentModel%Nodes%y, CurrentModel%Nodes%z, ierr)     

          DEALLOCATE( Owner )
          IF (ierr<0) THEN
            CALL Fatal('SParIterSolver','Failed to construct TrilinosKokkos solver')
          ELSE IF (ierr>0) THEN
            CALL Warn('SParIterSolver','Warning issued when trying to construct TrilinosKokkos solver')          
          END IF
        END IF
        ! solve using previously computed TrilinosKokkos data structures.
        ! NOTE: this is only correct if the matrix has not changed,
        ! otherwise we should use the SolveTrilinosKokkos3 function, which is
        ! not implemented, yet. This function will not update the matrix
        ! in the solver and thus solve an old system if A has changed. 
        CALL SolveTrilinosKokkos2( n, Xvec, RHSvec, &
                Rounds, TOL, verbosity, SourceMatrix % TrilinosKokkos, ierr)
      
      IF (ierr<0) THEN
        CALL Fatal('SParIterSolver',&
               'Linear system solve using TrilinosKokkos caused an error');
      ELSE IF (ierr>0) THEN
        IF (ListGetLogical(Solver%Values,'Linear System Abort Not Converged',Found)) THEN
          CALL Fatal('SParIterSolver', &
                'Linear system solve using TrilinosKokkos issued a warning');
        ELSE
          CALL Error('SParIterSolver', &
                'Linear system solve using TrilinosKokkos issued a warning');
        END IF
