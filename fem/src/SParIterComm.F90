...
! *****************************************************************************/
! *
! * $Id: SParIterComm.src,v 1.39 2007/05/03 11:04:51 jpr Exp $
! *****************************************************************************
! *****************************************************************************/
! *  23/10/09:
! *  - Elmer 5.4.0 source file
! *****************************************************************************/ 
! *****************************************************************************/
! *  26/10/09:
! *  - Modification of SParIterBarrier to check error
! *****************************************************************************/ 
! *****************************************************************************/
! *  26/10/09:
! *  - Subroutine ExchangeIFQuant added for the exchange of interfaces values
! *****************************************************************************/ 
! *****************************************************************************/
! *  15/12/09:
! *  - Subroutine SParActiveIntegerSUM: sum of integer values between all procs
! *    (use of MPI_ALLREDUCE)
! *****************************************************************************/ 
! *****************************************************************************/
! *  09/12/10:
! *  - Subroutine ExchangeIFQuant modified: only interface values are sent
! *****************************************************************************/ 
...
!-----------------------------------------------------------------------
    ...
    CALL MPI_INIT( ierr )
    IF ( ierr /= 0 ) THEN
       PRINT*,"Error of MPI_INIT = ",ierr
       RETURN
    END IF

    CALL MPI_COMM_SIZE( MPI_COMM_WORLD, ParEnv % PEs, ierr )
    IF ( ierr /= 0 .OR. ParEnv % PEs <= 1 ) THEN
       PRINT*,"Error of MPI_COMM_SIZE = ",ierr
       PRINT*,"Result of MPI_COMM_SIZE = ",ParEnv % PEs            
...
20  CONTINUE
    ...
    IF ( ierr /= 0 ) THEN
      PRINT *, 'MPI barrier failed ! (ierr=', ierr, ')'  
    END IF
!--------------------------------------------------------------------  
...
!*********************************************************************
!
! Exchange interfaces quantities with neighbours 
!
!*********************************************************************
SUBROUTINE ExchangeIFQuant( Model , ParallelInfo, Quant, SentQuant, Reorder, NDOFs, NDIM )
!*********************************************************************
    USE types
    IMPLICIT NONE
!*********************************************************************
    TYPE(Model_t) :: Model
    TYPE (ParallelInfo_t) :: ParallelInfo   
    REAL(KIND=dp) :: Quant(:), SentQuant(:)
    INTEGER ::  Reorder(:),NDOFs, NDIM
!*********************************************************************
    !------------------------------------------------------------------------------
    !   Local variables
    !------------------------------------------------------------------------------
    INTEGER :: i, j, k, m, k_dof, ierr, p, nbnodes, kmax, localIndexMax, src, &
        NbSentIfNodes, NbRecvIfNodes
    INTEGER, ALLOCATABLE :: LocalIndex(:), LocalI(:), SentIfQuantInd(:), RecvIfQuantInd(:)
    INTEGER, DIMENSION(MPI_STATUS_SIZE) :: status
    REAL(KIND=dp), ALLOCATABLE :: RecvQuant(:), SentIfQuantVal(:), RecvIfQuantVal(:)
    !------------------------------------------------------------------------------
    !   Computation of Global-local indexes correspondance in  partitioned mesh
    !------------------------------------------------------------------------------
    localIndexMax = MAXVAL(ParallelInfo % GlobalDOFs)
    ALLOCATE( LocalIndex( localIndexMax ) )
    !------------------------------------------------------------------------------
    !   Non-interface nodes have the index -1
    !------------------------------------------------------------------------------
    LocalIndex = -1
    NbSentIfNodes = 0   
    DO i =1,Model % NumberOfNodes
        !------------------------------------------------------------------------------
        ! If it is an interface node, get the global index:
        !------------------------------------------------------------------------------
        IF( .NOT. ParallelInfo % INTERFACE(i) ) CYCLE
        k = ParallelInfo % GlobalDOFs(i)
        !------------------------------------------------------------------------------
        ! By a bisection method, get LocalIndex(global index)=local index
        !------------------------------------------------------------------------------
        LocalIndex(k) = SearchNode(ParallelInfo, k) 
        !------------------------------------------------------------------------------
        ! Number of interface nodes:
        !------------------------------------------------------------------------------
        NbSentIfNodes = NbSentIfNodes + 1
    !------------------------------------------------------------------------------
    END DO
    !------------------------------------------------------------------------------
    ! Select the interface nodes only to be sent (values + indexes):
    !------------------------------------------------------------------------------
    ALLOCATE( SentIfQuantVal(NDIM*NDOFs*NbSentIfNodes), SentIfQuantInd(NDIM*NDOFs*NbSentIfNodes) )
    j = 1
    DO i=1,Model % NumberOfNodes
        IF (ParallelInfo % INTERFACE(i) ) THEN
            ...
            DO k_dof=1,NDOFs
                DO m=1,NDIM
                    SentIfQuantVal(NDOFs*(NDIM*(j-1)+m-1)+k_dof) = &
                        SentQuant(NDOFs*(NDIM*(SearchNode(ParallelInfo, k)-1)+m-1)+k_dof)
                    SentIfQuantInd(NDOFs*(NDIM*(j-1)+m-1)+k_dof) = &
                        NDOFs*(NDIM*(SearchNode(ParallelInfo, k)-1)+m-1)+k_dof
                END DO
            END DO
            j = j+1
        END IF
    ...
    CALL CheckBuffer(10000000)      
    !------------------------------------------------------------------------------
    !   Synchronisation of processus
    !------------------------------------------------------------------------------ 
    CALL SParIterBarrier
    !------------------------------------------------------------------------------
    ! Go through the processus and send our contribution
    ! on interface nodes
    !------------------------------------------------------------------------------
    DO p=0,ParEnv % Pes-1           
        !------------------------------------------------------------------------------
        !   Test if it is a neighbour:
        !------------------------------------------------------------------------------
        IF ( ParEnv % IsNeighbour(p+1) ) THEN
            !------------------------------------------------------------------------------
            !   Number of nodes in our processus:
            !------------------------------------------------------------------------------
            CALL MPI_BSEND(Model % NumberOfNodes, 1, MPI_INTEGER, p, 101, MPI_COMM_WORLD,ierr)
            IF ( ierr /= 0 ) PRINT *, 'MPI BSEND failed ! (ierr=', ierr, ')' 
            !------------------------------------------------------------------------------
            !   Number of interface nodes in our processus:
            !------------------------------------------------------------------------------
            CALL MPI_BSEND(NbSentIfNodes, 1, MPI_INTEGER, p, 100, MPI_COMM_WORLD,ierr)
            IF ( ierr /= 0 ) PRINT *, 'MPI BSEND failed ! (ierr=', ierr, ')'  
            !------------------------------------------------------------------------------
            !   Max global index of nodes in our processus:
            !------------------------------------------------------------------------------
            CALL MPI_BSEND(localIndexMax, 1, MPI_INTEGER, p, 99, MPI_COMM_WORLD,ierr)
            IF ( ierr /= 0 ) PRINT *, 'MPI BSEND failed ! (ierr=', ierr, ')'  
            !------------------------------------------------------------------------------
            !   Local indexes of nodes in our processus:
            !------------------------------------------------------------------------------
            CALL MPI_BSEND(LocalIndex, localIndexMax, MPI_INTEGER, p, 98, MPI_COMM_WORLD,ierr)
            IF ( ierr /= 0 ) PRINT *, 'MPI BSEND failed ! (ierr=', ierr, ')'  
            !------------------------------------------------------------------------------
            !   Value on interface nodes in our processus:
            !------------------------------------------------------------------------------
            CALL MPI_BSEND(SentIfQuantVal, NDIM*NDOFs*NbSentIfNodes, MPI_DOUBLE_PRECISION, p, 97, MPI_COMM_WORLD,ierr)
            IF ( ierr /= 0 ) PRINT *, 'MPI BSEND failed ! (ierr=', ierr, ')' 
            !------------------------------------------------------------------------------
            !   Index on interface nodes in our processus:
            !------------------------------------------------------------------------------
            CALL MPI_BSEND(SentIfQuantInd, NDIM*NDOFs*NbSentIfNodes, MPI_INTEGER, p, 96, MPI_COMM_WORLD,ierr)
            IF ( ierr /= 0 ) PRINT *, 'MPI BSEND failed ! (ierr=', ierr, ')'  
        !------------------------------------------------------------------------------
        END IF 
    !------------------------------------------------------------------------------
    END DO
    !------------------------------------------------------------------------------
    ! Go through the processus and get their contribution
    ! on interface nodes
    !------------------------------------------------------------------------------
    DO p=0,ParEnv % NumOfNeighbours-1   
        !------------------------------------------------------------------------------
        !   Number of nodes in source processus:
        !------------------------------------------------------------------------------
        CALL MPI_RECV(nbnodes, 1, MPI_INTEGER, MPI_ANY_SOURCE, 101, MPI_COMM_WORLD,status,ierr)
        IF ( ierr /= 0 ) PRINT *, 'MPI RECV failed ! (ierr=', ierr, ')'
        !------------------------------------------------------------------------------
        !   Keep the same source for the other recv:
        !------------------------------------------------------------------------------
        src = status(MPI_SOURCE)
        !------------------------------------------------------------------------------
        !   Number of interface nodes in source processus:
        !------------------------------------------------------------------------------
        CALL MPI_RECV(NbRecvIfNodes, 1, MPI_INTEGER, src, 100, MPI_COMM_WORLD,status,ierr)
        IF ( ierr /= 0 ) PRINT *, 'MPI RECV failed ! (ierr=', ierr, ')'
        !------------------------------------------------------------------------------
        !   Max global index of nodes in source processus:
        !------------------------------------------------------------------------------
        CALL MPI_RECV(kmax, 1, MPI_INTEGER, src, 99, MPI_COMM_WORLD,status,ierr)
        IF ( ierr /= 0 ) PRINT *, 'MPI RECV failed ! (ierr=', ierr, ')'  
        !------------------------------------------------------------------------------
        !   Local indexes of nodes in source processus:
        !------------------------------------------------------------------------------
        ALLOCATE( LocalI( kmax ) )
        CALL MPI_RECV(LocalI, kmax, MPI_INTEGER, src, 98, MPI_COMM_WORLD,status,ierr)
        IF ( ierr /= 0 ) PRINT *, 'MPI RECV failed ! (ierr=', ierr, ')'  
        !------------------------------------------------------------------------------
        !   Value and index on interface nodes in source processus:
        !------------------------------------------------------------------------------
        ALLOCATE( RecvIfQuantVal(NDIM*NDOFs*NbRecvIfNodes),RecvIfQuantInd(NDIM*NDOFs*NbRecvIfNodes) )

        CALL MPI_RECV(RecvIfQuantVal, NDIM*NDOFs*NbRecvIfNodes, MPI_DOUBLE_PRECISION, src, 97, &
            MPI_COMM_WORLD,status,ierr)
        IF ( ierr /= 0 ) PRINT *, 'MPI RECV failed ! (ierr=', ierr, ')'

        CALL MPI_RECV(RecvIfQuantInd, NDIM*NDOFs*NbRecvIfNodes, MPI_INTEGER, src, 96, &
            MPI_COMM_WORLD,status,ierr)
        IF ( ierr /= 0 ) PRINT *, 'MPI RECV failed ! (ierr=', ierr, ')'  
        !------------------------------------------------------------------------------
        !   Fill RecvQuant with interface values:
        !------------------------------------------------------------------------------
        ALLOCATE(RecvQuant(NDIM*NDOFs*nbnodes))
        RecvQuant = 0.0
        DO j=1,NDIM*NDOFs*NbRecvIfNodes
            RecvQuant(RecvIfQuantInd(j)) = RecvIfQuantVal(j) 
        END DO
        !------------------------------------------------------------------------------
        ! Add the contribution (RecvQuant) to Quant:            
        !------------------------------------------------------------------------------         
        DO i = 1, Model % NumberOfNodes
            !------------------------------------------------------------------------------
            ! Exchange only interface values
            !------------------------------------------------------------------------------
            IF( .NOT. ParallelInfo % INTERFACE(i) ) CYCLE
            !------------------------------------------------------------------------------
            ! Get the global correspondant index
            !------------------------------------------------------------------------------
            m = ParallelInfo % GlobalDOFs(i)
            !------------------------------------------------------------------------------
            ! If m > recieved global indexes, we won't find the information this time
            !------------------------------------------------------------------------------
            IF ( m <= kmax ) THEN
                !------------------------------------------------------------------------------
                ! This node should be also an interface node in the other partition
                !------------------------------------------------------------------------------
                IF (LocalI(m)>0) THEN
                    DO k_dof =1,NDOFs
                        DO j=1,NDIM
                            k = NDIM*(Reorder(i)-1)+j
                            Quant(NDOFs*(k-1)+k_dof) = Quant(NDOFs*(k-1)+k_dof) + &
                                RecvQuant(NDOFs*((NDIM*(LocalI(m)-1)+j)-1)+k_dof)
                        END DO
                    END DO !k_dof
                END IF
            END IF          
        END DO
        !------------------------------------------------------------------------------
        DEALLOCATE(RecvQuant, RecvIfQuantVal, RecvIfQuantInd, LocalI)
    !------------------------------------------------------------------------------
    END DO
    !------------------------------------------------------------------------------
    !   Synchronisation of processus
    ...
    CALL SParIterBarrier
    !------------------------------------------------------------------------------
    DEALLOCATE(LocalIndex, SentIfQuantVal, SentIfQuantInd)
!*********************************************************************
END SUBROUTINE ExchangeIFQuant
!*********************************************************************
...
!*********************************************************************
SUBROUTINE SParActiveIntegerSUM(tsum)
   INTEGER :: tsum
!*********************************************************************
   REAL(KIND=dp) :: s,RealTime
   INTEGER :: ssum,i,n,twop,nActive,myid,ierr
...
!s = RealTime()
   IF ( COUNT(ParEnv % Active)==ParEnv % PEs ) THEN
     ssum = tsum
     CALL MPI_ALLREDUCE( ssum, tsum, 1, MPI_INTEGER, &
...
!yyy = yyy + RealTime()-s
   ...
   twop = 1
   DO i=1,n
     IF ( MOD(myid,2*twop)==0 ) THEN
       IF (  myid+twop<nActive ) THEN
         CALL MPI_RECV(ssum, 1, MPI_INTEGER, &
            ActiveId(myid+twop), 7000, MPI_COMM_WORLD, status, ierr)
         tsum = tsum + ssum
       END IF
     ELSE
       CALL MPI_BSEND(tsum, 1, MPI_INTEGER, &
          ActiveId(myid-twop), 7000, MPI_COMM_WORLD, status, ierr)
       EXIT        
     END IF
     twop = twop*2
   ...
   DO i=n,1,-1
     ...
     ELSE IF ( MOD(myid,twop)==0 ) THEN
       CALL MPI_RECV(tsum, 1, MPI_INTEGER, &
          ActiveId(myid-twop), 7001, MPI_COMM_WORLD, status, ierr)
     END IF
     twop = twop/2
   END DO
!yyy = yyy + RealTime()-s
!*********************************************************************
END SUBROUTINE SParActiveIntegerSUM
!*********************************************************************
