...
MODULE IterSolve
   ...
   INTEGER, PARAMETER, PRIVATE :: ITER_CudaBiCGStab =           420
...
!------------------------------------------------------------------------------
    ...
    REAL(KIND=dp) :: dpar(50),stopfun,frobenius
!   external stopfun
...
!------------------------------------------------------------------------------
    ...
    ELSE IF ( str(1:3) == 'gcr' ) THEN
      IterType = ITER_GCR
    ELSE IF ( str(1:12) == 'cudabicgstab' ) THEN
      IterType = ITER_CudaBiCGStab
    ELSE
      IterType = ITER_BiCGStab
    END IF
...
!------------------------------------------------------------------------------
    ...
    IF ( .NOT.Gotit ) HUTI_TOLERANCE = 1.0e-9
...
!------------------------------------------------------------------------------
    ...
    IF(stack_pos>stack_max) THEN
      CALL Fatal('IterSolver', 'Recursion too deep')
    ...
    GlobalMatrix => A
    PRINT *, "Rows ", SIZE( A % Rows ) 
    PRINT *, "Values", SIZE( A % Values ) 

    frobenius = 0._dp
    DO i=1,SIZE(A % Values)
        frobenius = frobenius + A % Values(i) * A % Values(i)
    END DO
    PRINT *, "A Frobenius ", SQRT(frobenius)
    frobenius = 0._dp

    DO i=1,A % NumberOfRows
        frobenius = MAX(frobenius, ABS(x(i)))
    END DO
    PRINT *, "x ", frobenius
    frobenius = 0._dp

    DO i=1,A % NumberOfRows
        frobenius = MAX(frobenius, ABS(b(i)))
    END DO
    PRINT *, "b ", frobenius
