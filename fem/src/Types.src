...
END INTERFACE
  ...
  TYPE Matrix_t
    TYPE(Matrix_t), POINTER :: Child, Parent, &
        ConstraintMatrix, EMatrix, AddMatrix=>NULL(), CollectionMatrix=>NULL()
    ...
    INTEGER, POINTER :: Perm(:),InvPerm(:), Gorder(:), EPerm(:)
    INTEGER, ALLOCATABLE :: GRows(:), RowOwner(:)
    INTEGER, POINTER CONTIG :: Rows(:),Cols(:), Diag(:)

    REAL(KIND=dp), POINTER CONTIG :: RHS(:),BulkRHS(:),RHS_im(:),Force(:,:)
    REAL(KIND=dp), POINTER CONTIG :: BulkResidual(:)

    REAL(KIND=dp),  POINTER CONTIG :: Values(:), ILUValues(:), &
               DiagScaling(:) => NULL()

    REAL(KIND=dp), ALLOCATABLE :: extraVals(:)
    REAL(KIND=dp) :: RhsScaling
    REAL(KIND=dp),  POINTER CONTIG :: MassValues(:),DampValues(:), &
                   BulkValues(:), PrecValues(:)=>Null()
...
#endif
#ifdef HAVE_TRILINOS
    INTEGER(KIND=C_INTPTR_T) :: TrilinosEpetra=0
    INTEGER(KIND=C_INTPTR_T) :: TrilinosKokkos=0
#endif
    INTEGER(KIND=AddrInt) :: SpMV=0

    INTEGER(KIND=AddrInt) :: MatVecSubr = 0

    INTEGER, POINTER CONTIG :: ILURows(:),ILUCols(:),ILUDiag(:)
