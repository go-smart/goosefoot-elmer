...
!-------------------------------------------------------------------------------
       ...
       REAL(KIND=dp), POINTER :: store(:), Axisy
...
!------------------------------------------------------------------------------
         ...
         IF (PRESENT(AffineBackTransformation)) THEN
!        WRITE(*,*) "Before Projection",Point
           NewPoint = Point
           Point(1) = SUM(AffineBackTransformation(1:3,1) * NewPoint) + AffineBackTransformation(4,1)
           Point(2) = SUM(AffineBackTransformation(1:3,2) * NewPoint) + AffineBackTransformation(4,2)
           Point(3) = SUM(AffineBackTransformation(1:3,3) * NewPoint) + AffineBackTransformation(4,3)
         END IF

         IF (CylindricSymmetricSolver) THEN
           Point(1) = Point(1) ** 2 + Point(3) ** 2
           IF (Point(1) > 0._dp) THEN
             Point(1) = SQRT(Point(1))
           END IF
           Point(3) = 0._dp         
!           READ(*,*) Panch
         END IF
...
!------------------------------------------------------------------------------
             END IF
             Var => Var % Next
!             WRITE(353,*) Point,NewValue(NewSol % Perm(i))
           END DO
!------------------------------------------------------------------------------
         END DO
...
!RMV Panch        
!        Do i=1,NewMesh%NumberOfNodes
!             Panchvar(i,1)=NewMesh%Nodes%x(i)
!             Panchvar(i,2)=NewMesh%Nodes%y(i)
!             Panchvar(i,3)=NewMesh%Nodes%z(i)             
!        END DO        
!        k=4
!RMV Panch        
        DO WHILE( ASSOCIATED(Var) )
...
!RMV Panch                   
!         Do i=1,NewMesh%NumberOfNodes
!           Panchvar(i,k)=NewSol % Values(NewSol % Perm(i)) 
!         END DO 
!           k=k+1 
!RMV Panch             
        END DO
...
!       ind = 1
!        fileloop: do
!          write(buffer,"(A,I5.5,A)") "Projected", ind, ".dat"
...
!          ind = ind + 1
!        end do fileloop
!       WRITE(*,*) "Enter the file number ",PanchFile        
!       OPEN(353, FILE=PanchFile)
!       Do i=1,NewMesh%NumberOfNodes
!           WRITE(353,*) Panchvar(i,1:k)
!       END DO   
!RMV Panch        
