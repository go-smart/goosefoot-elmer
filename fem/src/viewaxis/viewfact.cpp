  ...
  (int *n,int *surf, Real *crd, Real *vf, int *idiv, int *fast)
{
  ...
  for (i=0; i<nsurf; i++) {
	...
	for (jj=0; jj<div; jj++) {
	  ...
	  /* Kerroin 4 koostuu tekijˆist‰ 2 (peilisymmetria), 2pi */
...
Real ViewIntegral (Real c1, Real c2, int k)
{
  /*
    ...
    kutsu molemmille osille. Jos integrointialue kutistuu mit‰ttˆm‰ksi
  ...
  while (k < nsurfShade) {
    ...
    if ( fabs(zd5) < eps ) {
      ...
      /* Varjostava pinta on kartio tai lieriˆ       */
	...
	if (t0 - t1 > eps && t2 - t0 > eps) {
      ...
      if (cc1 > cc2) {
	cc1 = cc2; /* N‰in voi k‰yd‰ pyˆristysvirheiden takia */
      }
...
BOOL IntervalIsect(Real x1, Real x2, Real y1, Real y2, Real *z1, Real *z2)
{
  ...
  /* t‰m‰ on tyhj‰ tai mit‰tˆn. Input-parametrien j‰rjestyksen on oltava */
  /* oikea.*/
